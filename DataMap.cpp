...
extern double reaction_time_take_control;
extern double reaction_time_give_control;

//Implemention of extern variable in Vehicle_data.h

/* This stores certain vehicle informations for each vehicle for a particular time step*/
/* This data is stored via Simulation_Data.cpp in a csv file */
	...
	char   *string_value)
{
	...
	//For each peice of vehicle data provided by VISSIM, update assosiative array
...
#if defined(SAE5_CAR) || defined(SAE5_TRUCK) || defined(SAE4_CAR) || defined(SAE5_TRUCK)
		DataMap[VehicleID].level_shift = Automated_Control;
		break;
#endif
		/*if (cur_link==21 || cur_link!= 30 || cur_link != 29 || cur_link != 8 || cur_link != 9 ||
			cur_link == 20 || cur_link != 25 || cur_link != 23 || cur_link != 7 || cur_link != 35 ||
			cur_link == 53 || cur_link != 59 || cur_link != 54 || cur_link != 58 || cur_link != 57 ||
			cur_link == 56 || cur_link != 55 || cur_link != 52 || cur_link != 2 || cur_link != 60 ||
			cur_link == 61 || cur_link != 64 || cur_link != 65 || cur_link != 66 || cur_link != 22 ||
			cur_link == 68 || cur_link != 69
			)
		{
...
//		}vehcieldata
//#else
//	    if (DataMap[VehicleID].level_shift == Automated_Control && DataMap[VehicleID].current_velocity > 30)
		if (!utils::NormalDistribution::isVehicleOnHighway(&DataMap[VehicleID]))
		{
			DataMap[VehicleID].time_to_shift = DataMap[VehicleID].current_time + reaction_time_take_control;
			DataMap[VehicleID].level_shift = Human_Control;
		}
		//check to see if we are under human control and below 17.9 m/s speed limit
//		else if (DataMap[VehicleID].level_shift == Human_Control && DataMap[VehicleID].current_velocity < 30)
		else
		{
			DataMap[VehicleID].time_to_shift = DataMap[VehicleID].current_time + reaction_time_give_control;
			DataMap[VehicleID].level_shift = Automated_Control;
		}
//#endif
	...
	case DRIVER_DATA_VEH_REAR_X_COORDINATE:
		DataMap[VehicleID].x_coordinate_rear = double_value;
		break;
	case DRIVER_DATA_VEH_REAR_Y_COORDINATE:
		DataMap[VehicleID].y_coordinate_rear = double_value;
		break;
	...
	case DRIVER_DATA_VEH_LANE:
		...
		DataMap[VehicleID].cur_veh_lane = long_value;
		break;
	case DRIVER_DATA_VEH_CURRENT_LINK:
		...
		DataMap[VehicleID].cur_link = long_value;
		break;
	case DRIVER_DATA_VEH_LENGTH:
		DataMap[VehicleID].length = double_value;
		break;
	case DRIVER_DATA_VEH_WIDTH:
		DataMap[VehicleID].width = double_value;
		break;
...
void StoreSituationData(int Veh)
{
	...
	std::string filename3 = "U:\\DataLog\\" + std::string(data) + "_DEBUG_" + Time + ".csv";
